...
HexagonEngine& HexagonEngine::Get() { return *loadedEngine; }
void HexagonEngine::init()
{
...
void HexagonEngine::draw_background(VkCommandBuffer cmd)
{
    ...
    VkDescriptorSet imageSet = get_current_frame()._frameDescriptors.allocate(_device, _drawImageDescriptorLayout);
	...
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, _gradientPipelineLayout, 0, 1, &imageSet, 0, nullptr);
...
void HexagonEngine::draw_mesh(VkCommandBuffer cmd)
{
    ...
    imageSet = get_current_frame()._frameDescriptors.allocate(_device, _singleImageDescriptorLayout);
...
void HexagonEngine::init_swapchain()
{
...
void HexagonEngine::init_commands()
{
...
void HexagonEngine::init_sync_structures()
{
...
void HexagonEngine::create_swapchain(uint32_t width, uint32_t height)
{
...
void HexagonEngine::destroy_swapchain()
{
...
void HexagonEngine::init_descriptors()
{
    
    {
        DescriptorLayoutBuilder builder;
        builder.add_binding(0, 1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
        _drawImageDescriptorLayout = builder.build(_device, VK_SHADER_STAGE_COMPUTE_BIT);
    }

    {
        DescriptorLayoutBuilder builder;
        builder.add_binding(0, ARRAY_MAX_TEXTURE_COUNT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
...
void HexagonEngine::init_background_pipelines()
{
...
void HexagonEngine::draw_imgui(VkCommandBuffer cmd, VkImageView targetImageView)
{
...
AllocatedBuffer HexagonEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
{
    // allocate buffer
...
void HexagonEngine::destroy_buffer(const AllocatedBuffer& buffer)
{
...
void HexagonEngine::immediate_submit(std::function<void(VkCommandBuffer cmd)>&& function)
{
...
void HexagonEngine::resize_swapchain()
{
    vkDeviceWaitIdle(_device);
...
AllocatedImage HexagonEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
...
AllocatedImage HexagonEngine::create_image(void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
...
void HexagonEngine::destroy_image(const AllocatedImage& img)
{
...
AllocatedImage HexagonEngine::loadTexture(const char* texturePath)
{
    ...
    unsigned char* data = stbi_load(texturePath, &width, &height, &nrComponents, 4);
...
void HexagonEngine::init_sampler()
{
